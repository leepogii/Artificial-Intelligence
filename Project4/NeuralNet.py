...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for w in self.weights:
            modification = alpha * delta * inActs[counter]
            self.weights[counter] = w + modification
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                perceptrons.append(perceptron.sigmoidActivation(newActs))

            newActs = perceptrons
            returnList.append(perceptrons)
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    weightSum = 0
                    for i in range(len(deltas[layerNum])):
                        weightSum += nextLayer[i].weights[neuronNum + 1] * deltas[layerNum][i]
                    delta = gPrime * weightSum
