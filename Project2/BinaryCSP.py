...
def consistent(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if bc.affects(var) and not bc.isSatisfied(value, assignment.assignedValues[bc.otherVariable(var)]):
			return False
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in assignment.assignedValues:
		if assignment.assignedValues[var] is None:
			if nextVar is None:
				nextVar = var
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	values = helperFunc(assignment, csp, var, assignment.varDomains[var])

	return values

def helperFunc(assignment, csp, var, values):
	...
	for val in values:
		...
		for bc in csp.binaryConstraints:
			if bc.affects(var):
				if val in assignment.varDomains[bc.otherVariable(var)]:
					count += 1
		newVal.append((val, count))
	newVal = sorted(newVal, key=lambda tup: tup[1])
	toReturn = []

	for val in newVal:
		toReturn.append(val[0])
...
def forwardChecking(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if bc.affects(var):
			for aVal in list(assignment.varDomains[bc.otherVariable(var)]):
				if not bc.isSatisfied(value, aVal):
					inferences.add((bc.otherVariable(var), aVal))
					assignment.varDomains[bc.otherVariable(var)].remove(aVal)
					if not assignment.varDomains[bc.otherVariable(var)]:
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for val in orderValuesMethod(assignment, csp, var):
		inference = inferenceMethod(assignment, csp, var, val)
		if consistent(assignment, csp, var, val):
			assignment.assignedValues[var] = val
			result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
			if result is not failure:
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for v2 in assignment.varDomains[var2]:
		for v1 in assignment.varDomains[var1]:
			if len(assignment.varDomains[var1]) == 1 and not constraint.isSatisfied(v1, v2):
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if bc.affects(var):
			dq.append((var, bc.otherVariable(var), bc))

	while len(dq) != 0:
		var1, var2, constraint = dq.pop()
		vals = revise(assignment, csp, var1, var2, constraint)
		if vals is None:
			for inf in inferences:
				assignment.varDomains[inf[0]].add(inf[1])
			return None
		for inf in vals:
			inferences.add(inf)
			for bc in csp.binaryConstraints:
				if bc.affects(var2) and not bc.affects(var1):
					dq.append((var2, bc.otherVariable(var2), constraint))
