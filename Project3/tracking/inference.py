...
class ParticleFilter(InferenceModule):
	...
	def initializeUniformly(self, gameState):
		...
		for lp in self.legalPositions:
			newParticles.append(lp)
		count = 0
		i = 0
		while True:
			if i == len(newParticles):
				i = 0
			if count==self.numParticles:
				break
			self.particles.append(newParticles[i])
			count += 1
			i += 1
	...
	def observe(self, observation, gameState):
		...
		if noisyDistance is None:
			self.particles = [self.getJailPosition()] * numParticle

		else:
			for particle in self.particles:
				dist = util.manhattanDistance(pacmanPosition, particle)
				allPosible[particle] += emissionModel[dist]

			if allPosible.totalCount() > 0:
				for i in range(self.numParticles):
					self.particles[i] = util.sample(allPosible)
...
class JointParticleFilter:
	...
	def initializeParticles(self):
		...
		"""
		self.particles=[]
		newPar = itertools.product(self.legalPositions, repeat=self.numGhosts)
		newParticles = []
		for par in newPar:
			newParticles.append(par)
		...
		for particle in self.particles:
			...
			for i in range(numGhost):
				if noisyDistances[i] is None:
					...
					prob = prob * emissionModels[i][dist]
	...
	def elapseTime(self, gameState):
		...
		**Note** that you may need to replace "prevGhostPositions" with the
		...
		for oldPar in self.particles:
			newPar = list(oldPar) 
			for i in range(self.numGhosts):
				newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newPar), i,
				                                             self.ghostAgents[i])
				newPar[i] = util.sample(newPosDist)
			newPars.append(tuple(newPar))
